...
### 5.맵 디자인 요구사항
5-1 1스테이지 컨셉인 실험실 레벨디자인을 에셋을 활용하여 구현한다.

### 6. 기타 요구사항

6-1 스테이지는 없으며, 게임을 진행할때마다 함정이 자주 등장한다.
...
1-1 ~~시작화면, 게임화면 총 2개의 화면이 있다.~~ (3-1 임형균)

1-2 ~~시작화면에는 시작하기 버튼 1개가 있다.~~ (3-2 임형균)

1-3 ~~시작하기 클릭 시 게임화면으로 이동한다.~~ (3-3 임형균)

1-4 ~~Plastic SCM으로 프로젝트 공유하기~~ (이승현)

### 2주차

~~2-1 조작은 키보드 상하좌우 화살표로 이동, Spacebar 점프 (1-14 임형균)~~

~~2-2 게임 진행 중 ESC키를 누르면 환경설정 창이 나온다. (3-8 이승현)~~

~~2-3 환경설정 창이 표시되어 있는 경우 돌아가기 버튼을 누르거나 ESC키를 재입력하면 환경설정 창이 닫힌다. (4-3 이승현)~~

~~2-4 조작법을 누르면 게임 내에서 사용 가능한 키보드 입력 값과 설명팝업이 화면에 표시된다. (4-4 이승현)~~

~~2-5 환경설정 창에서는 사운드 조절과 게임종료, 돌아가기, 조작법을 보여준다. (4-5 이승현)~~

~~2-6 게임종료 버튼 클릭시 '현재 진행된 부분까지 저장됩니다'라는 팝업이 표시되며 확인 버튼을 누르면 게임이 저장된다. (3-6 임형균)~~

### 3주차

3-1 함정에 충돌시 사망하며 Life가 1 감소된다. (1-3 임형균)

3-2 사망 후 남은 Life가 0이라면 게임의 처음으로 되돌아가게 되며, 남은 Life의 개수가 0을 초과한다면 마지막 SavePoint로 이동하게된다. (1-13 임형균)

3-3 플레이어가 SavePoint존에 들어가게되면 현재 SavePoint의 Position값과 구출한 동료가 저장된다. (3-4 이승현)

3-4 F키로 오브젝트 상호작용 (임형균)

3-5 게임 진행중 Life아이템이 나오는데, 프로틴통 모양의 오브젝트로 맵 곳곳에 배치되어있다. Life 오브젝트에 가까이 가서 F키로 상호작용시 Life가 1증가되고, 오브젝트는 사라진다. (1-2 이승현,임형균)

### 4주차

4-1 사운드 조절은 0에서 100까지 조절 가능하며 음소거ON/OFF 기능이 있다. (3-5 이승현)

4-2 선풍기 장애물은 강한 바람을 일으켜 플레이어를 일정 방향으로 밀어낸다 (2-3 이승현)

4-3 게임화면에는 왼쪽 하단에 플레이어 초상화, 왼쪽상단에 동료초상화, 우측상단에 Life가 있다. (4-1 임형균)

4-4 1스테이지 컨셉인 실험실 레벨디자인을 에셋을 활용하여 구현한다. (4-2 이승현)

4-5 동료를 구출하기 위해 문 앞에서 F키를 이용하여 상호작용하게 되면 타이밍체크 팝업창이 뜨게되며 Space바를 이용해 일정한 칸에 맞추게 되면 타이밍체크 팝업창이 꺼지고 비밀번호 팝업창이 뜬다. (3-9 임형균)
