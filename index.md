...
•게임 진행중 게임화면 중간하단에 Dialogue창이 나오고 F키를 이용하여 넘어갈 수 있다.

•게임 진행중 Life아이템이 나오는데, 프로틴통 모양의 오브젝트로 맵 곳곳에 배치되어있다. Life 오브젝트에 가까이 가서 F키로 상호작용시 Life가 1증가되고, 오브젝트는 사라진다.

•함정에 충돌시 사망하며 Life가 1 감소된다.

•깊은 낭떠러지는 양으로 떨어지지 않을 경우, 사망하며 Life가 1 감소된다.

•바다는 수중형 속성을 가진 동료 동물(오징어)만 지나갈 수 있으며,  그 외 속성의 동물로 들어갈 시 사망하며 Life가 1감소한다.

•바다 속 장애물로 상어가 존재하며 R키로 오징어의 특성을 이용하여 먹물을 쏴 명중시 무력화 시킨다.

•약한 지반 지형지물은 플레이어가 지나갈시 일정 시간 이후에 지반이 무너지기 시작한다.

•약한 지반 지형지물을 지나갈 때는 스피드형 속성을 가진 동물(사슴)을 이용하여 지나갈 수 있다.

•빙판 지형지물은 슬라이드형 속성(펭귄)을 가진 동료동물 지나갈 수 있으며 다른 동물로 입장할 시 Drag값이 증가하여 움직일 수 없으며 '추워서 못가겠어' 라는 Dialogue가 출력된다.

•정글짐 장애물은 곡예형 속성을 가진 동료 동물(원숭이)의 특성을 이용하여야만 넘어갈 수 있다.

•좁고 낮은 길은 크랙형 속성을 가진 동료동물(뱀)을 이용해야만 지나갈 수 있다.

•리스트에 추가된 동물로 둔갑하여 게임을 진행 할 수 있다.

•사망 후 남은 Life가 0이라면 게임의 처음으로 되돌아가게 되며, 남은 Life의 개수가 0을 초과한다면 마지막 SavePoint로 이동하게된다.

•조작은 키보드 상하좌우 화살표로 이동, Spacebar 점프, F키는 오브젝트 상호작용 및 Dialouge진행, R키는 동료특성사용, 키보드 상단 넘버키로 동료동물로 둔갑 할 수 있다.

### •장애물/함정 요구사항

•장애물과 함정, 특정지형지물로는 높은 벽, 좁고 낮은 길, 깊은 낭떠러지, 정글짐, 바다, 철퇴, 선풍기, 빙판길, 약한지반, 상어가 있다.

•철퇴 함정은 맵 중간중간 등장하며 진자운동을 하며 길을 막고 있으며 플레이어와 충돌시 사망하며 Life가 1 감소한다.

•선풍기 장애물은 강한 바람을 일으켜 플레이어를 일정 방향으로 밀어낸다.

•철퇴 함정 주변에는 레버 오브젝트가 존재하며 F키를 이용해 플레이어가 상호작용하여 철퇴의 작동을 멈출 수 있다.

•선풍기 장애물 주변에는 레버 오브젝트가 존재하며 F키를 이용해 플레이어가 상호작용하여 선풍기의 작동을 멈출 수 있다.

•게임 진행중 다음 구역으로 넘어가기 전 닫혀있는 문은 문 옆의 버튼 오브젝트에 F키로 상호작용하여 열 수 있다.

### •게임 시스템 요구사항

•시작화면, 게임화면 총 2개의 화면이 있다.

•시작화면에는 시작하기 버튼 1개가 있다.

•시작하기 클릭 시 게임화면으로 이동한다.

•플레이어가 SavePoint존에 들어가게되면 현재 SavePoint의 Position값과 구출한 동료가 저장된다.

•사운드 조절은 0에서 100까지 조절 가능하며 음소거ON/OFF 기능이 있다.

•게임종료 버튼 클릭시 '현재 진행된 부분까지 저장됩니다'라는 팝업이 표시되며 확인 버튼을 누르면 게임이 저장된다.

•게임 종료시 Dialogue와 이미지가 나오며 엔딩장면이 나온다.

•게임 진행 중 ESC키를 누르면 환경설정 창이 나온다.

•동료를 구출하기 위해 문 앞에서 F키를 이용하여 상호작용하게 되면 타이밍체크 팝업창이 뜨게되며 Space바를 이용해 일정한 칸에 맞추게 되면 타이밍체크 팝업창이 꺼지고 비밀번호 팝업창이 뜬다.

•게임의 종료 조건은 모든 동료를 구출 후 박사의 실험실에서 탈출하면 종료된다.

### •UI 요구사항

•게임화면에는 왼쪽 하단에 플레이어 초상화, 왼쪽상단에 동료초상화, 우측상단에 Life가 있다.

•비밀번호 팝업창은 갇혀있는 동물의 영어 이름을 입력한 후 확인 버튼을 누르면 팝업창이 닫히게되고 동료 동물 리스트에 추가된다.

•환경설정 창이 표시되어 있는 경우 돌아가기 버튼을 누르거나 ESC키를 재입력하면 환경설정 창이 닫힌다.

•조작법을 누르면 게임 내에서 사용 가능한 키보드 입력 값과 설명팝업이 화면에 표시된다.

•환경설정 창에서는 사운드 조절과 게임종료, 돌아가기, 조작법을 보여준다.

### •기타 요구사항

•스테이지는 없으며, 게임을 진행할때마다 함정이 자주 등장한다.
